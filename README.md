...
2. **Use in your code**:
   ...
   // Get version info
   const {dynamicVersion, versions, buildNumber} = readDynamicVersion();
...
```json
{
  ...
  "baseVersion": "0.1.0",
  "branch": "main",
  "buildNumber": "20251021.123456.78",
  "commitsSince": 5,
  "dirty": false,
  "dynamicVersion": "0.1.5",
  "generationTrigger": "git-hook",
  "timestamp": "Tue Oct 21 2025 12:34:56 GMT-0700 (Pacific Daylight Time)",
  "timestampUnix": 1761075296789,
  "versions": {
    "runtime": "0.1.0"
  }
}
...
- Contains `baseVersion` (from package.json) and computed `dynamicVersion` based on `commitsSince`, the commits since the last bump
- `versions` is copied from `version-manager.json`, and is `{}` when you track none
- `generationTrigger` is `git-hook` or `cli`
- `timestamp` is JavaScript's `Date#toString()` in local time, not ISO 8601; `timestampUnix` is milliseconds since the epoch
- Generation also writes `dynamic-version.local.d.ts` beside it, which types `versions` with the keys you configured. Pass `--no-types` to skip it
...
**Scripts added to package.json** (the exact commands are under [In package.json scripts](#in-packagejson-scripts)):

- `dynamic-version` and `dynamic-version:generate` - Generate version file manually
...
- `[versions..]` (positional): Names from the `versions` map in `version-manager.json` to set to the new version too, e.g. `bump runtime` or `bump runtime pancake`. Each name must already be in the map, or bump fails before writing anything. `event-log` mode syncs no named versions, so there any name fails
...
# Bump and update the runtime version too
npx @justinhaaheim/version-manager bump runtime --minor --commit
...
- **Where it lives**: the `runtime` entry of the `versions` map in `version-manager.json`, copied as-is into the generated file's `versions`. It is one named version like any other; the map may hold more
- **How to change it**: edit it by hand, or `bump runtime` to set it to the new version as you bump
...
A timestamp taken from the local clock each time the version file is generated.

- **Format**: `YYYYMMDD.HHmmss.SS`, local time, hundredths of a second last (e.g., `20251021.123456.78`)
- **Source**: always the clock. No environment variable is read: setting `BUILD_NUMBER` or a CI build number changes nothing
...
const { dynamicVersion } = readDynamicVersion();
...
`buildNumber` comes from the clock of the machine that generates the file. The tool reads no CI build-number variable, so there is nothing to set first.
...
- `prebuild`, `predev` and `prestart` run a plain generate, with no `--silent` and no `--no-fail`. If generating the version fails, the build, dev server or start fails with it.
- `prepare` carries `--no-fail`, so a failed generation does not fail `npm install`. If `package.json` already has a `prepare` script, the command is appended to it with `&&` rather than replacing it.
- `install` adds `prepare`, `prebuild`, `predev` and `prestart` in `dynamic-file` mode only. The other two modes write no generated file for them to regenerate.
...
declare module '*/dynamic-version.local.json' {
  interface DynamicVersion {
    ...
    timestampUnix: number; // milliseconds since the epoch
    versions: Record<string, string>; // e.g. {runtime: '0.1.0'}
  }
